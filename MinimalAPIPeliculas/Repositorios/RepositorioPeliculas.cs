    ...
    public class RepositorioPeliculas : IRepositorioPeliculas
    {
        ...
        public async Task<Pelicula?> ObtenerPorId(int id)
        {
            using (var conexion = new SqlConnection(connectionString))
            {
                ...
                //var pelicula = await conexion.QueryFirstOrDefaultAsync<Pelicula>("Peliculas_ObtenerPorId", new { id }, commandType: CommandType.StoredProcedure);
            }
        }
        public async Task<List<Pelicula>> ObtenerPorNombre(string nombre)
        {
            using (var conexion = new SqlConnection(connectionString))
            {
                var peliculas = await conexion.QueryAsync<Pelicula>("Peliculas_ObtenerPorNombre", new { nombre }, commandType: CommandType.StoredProcedure);
                return peliculas.ToList();
            }
        }
        public async Task<int> Crear(Pelicula pelicula)
        {
            using (var conexion = new SqlConnection(connectionString))
            {
                var id = await conexion.QuerySingleAsync<int>("Peliculas_Crear",
                    new { pelicula.Titulo, pelicula.FechaLanzamiento, pelicula.EnCines, pelicula.Poster },
                    commandType: CommandType.StoredProcedure);
                pelicula.Id = id;
                return id;
            }
        ...
        public async Task Actualizar(Pelicula pelicula)
        {
            using (var conexion = new SqlConnection(connectionString))
            {
                var id = await conexion.ExecuteAsync("Peliculas_Actualizar",
                    pelicula,
                    commandType: CommandType.StoredProcedure);
            }
        }
        ...
        public async Task Borrar(int id)
        {
            using (var conexion = new SqlConnection(connectionString))
            {
                await conexion.ExecuteAsync("Peliculas_Borrar",
                    new { id },
                    commandType: CommandType.StoredProcedure);
            }
        }
