...
namespace MinimalAPIPeliculas.Repositorios
{
    public class RepositorioActores : IRepositorioActores
    {
        ...
        public RepositorioActores(IConfiguration configuration, IHttpContextAccessor httpContextAccesor)
        {
        ...
        public async Task<List<Actor>> ObtenerTodos(PaginacionDTO paginacionDTO)
        {
            using (var conexion = new SqlConnection(connectionString))
            {
                ...
                var cantidadActores = await conexion.QuerySingleAsync<int>("Actores_Cantidad",
                    commandType: CommandType.StoredProcedure);
        ...
        public async Task Actualizar(Actor actor)
        {
            using (var conexion = new SqlConnection(connectionString))
            {
                var id = await conexion.ExecuteAsync("Actores_Actualizar",
                    actor,
                    commandType: CommandType.StoredProcedure);
            }
        }
